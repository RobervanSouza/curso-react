...
// 9 so consegue colocar o nome cadastrar porque dentra da input login esta usanda o CHILDREN 
  ...
  //useEffect (() =>{ // 3 o useeffect atualiza somento uma vez, quando iniciar, por exemplo em chamada de APi, que aconteçe somente uma vez 

//  },[])//6 [] quando deiaxa vazio ele inicia vazio e não altera nada. Quando coloca por exemplo uma dependencia[email], toda vez que coloca um email, ele altera
...
//const handleToOpen= useCallback(() => { // 3 quando inicia executa uma função, geralmente usa dois parametros, e como se fosse o useeffect

// 4 esse buton (type="button") aguarda uma função para poder executar a função, a função submit recarrega a pagina não precisa da função
...
//  const inputPasswords = useRef<HTMLInputElement>(null);// 4 quando iniciar o valor e nulo, depois que renderiza o html ele altera, e muito utilizado para pegar a referencia para alterar
 ...
 //const [email, setEmail] = useState('') // 3 useState valor inicial strig vazia. no input pega o valor do email. quando digita na input, sempre que atualiza começa sem valor a input ('')
